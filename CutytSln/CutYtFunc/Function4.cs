...
namespace CutYtFunc
{
    public class Function4
    {
        ...
        public Function4(IHostEnvironment host, IOptions<ExecutionContextOptions> executionContext)
        {
            this.host = host;
            ...
            ILogger log)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(name))
                {
                    finalArgs = $" {args} https://www.youtube.com/watch?v={name}";
                }

                var res = ProcessSyncHelper.ExecuteShellCommand(
                            $@"{path3}\youtube-dl.exe",
                            $" {args} https://www.youtube.com/watch?v={name}");
